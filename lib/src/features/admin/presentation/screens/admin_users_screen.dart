...
class AdminUsersScreen extends ConsumerWidget {
          ...
          child: ListView(
            children: [
              AdminAsyncList<AdminUserRow>(
                ...
                emptyMessage: 'Customer yang mendaftar akan tampil di sini.',
  ...
  Future<void> _showUserDetail(BuildContext context, WidgetRef ref, AdminUserRow user) async {
...
class _UserCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AppCard(
      ...
      child: Row(
        children: [
          IconBadge(icon: user.emailVerified ? Icons.verified_user_rounded : Icons.person_outline_rounded),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(user.name, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w900)),
...
class _UserDetailSheetState extends ConsumerState<_UserDetailSheet> {
  ...
  void initState() {
    super.initState();
    _detailFuture = ref.read(adminRepositoryProvider).userDetail(widget.user.id);
  ...
  Future<void> _delete() async {
    final ok = await confirmDanger(
      context,
      title: 'Hapus user?',
      message: 'User ${widget.user.name} akan dihapus jika belum punya riwayat booking.',
    );
    if (!ok) return;
    setState(() => _saving = true);
    try {
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Text('Detail User', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
            ...
            FutureBuilder<AdminUserDetail>(
              ...
              builder: (context, snapshot) {
                ...
                return Column(
                  children: [
                    AdminInfoRow(label: 'Total booking', value: '${data.totalBookings}'),
                    AdminInfoRow(label: 'Total belanja', value: formatRupiah(data.totalSpent)),
                    AdminInfoRow(label: 'Last activity', value: readableDateTime(data.lastActivityAt)),
                  ],
                );
              },
            ),
            const SizedBox(height: 12),
            AppTextField(controller: _nameController, label: 'Nama', prefixIcon: Icons.person_rounded),
            const SizedBox(height: 12),
            AppTextField(controller: _phoneController, label: 'No. HP', prefixIcon: Icons.phone_rounded, keyboardType: TextInputType.phone),
            const SizedBox(height: 12),
